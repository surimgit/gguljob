...
MAX_CONTENT = 3500
BATCH_SIZE = 15
MAX_CONCURRENT_REQUESTS = 6  # Rate limit safe  # 동시에 보낼 API 요청 수 (비동기)
...
async def process_batch(batch_df, sem):
    ...
    for idx, row in batch_df.iterrows():
        job_id = str(row['링크']).split(
            '/')[-1] if pd.notna(row['링크']) else str(idx)
...
async def main():
    ...
    print(f"Super Speed: Async {MAX_CONCURRENT_REQUESTS} Limits Running...")

    sem = asyncio.Semaphore(MAX_CONCURRENT_REQUESTS)
    tasks = [process_batch(b, sem) for b in batches]

    # 비동기로 동시에 쏟아붓기 (tqdm 포함)
