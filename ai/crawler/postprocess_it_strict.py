...
[절대 지켜야 할 원칙]
1. 공고가 여러 부서를 뽑는 '종합 채용 공고'(영업, 재무, 제조, IT 등 혼합)인 경우, **오직 IT 관련 부문**의 주요업무, 자격요건, 우대사항만 찾아내세요.
2. 절대 내용을 요약하거나 임의로 단어를 통합하지 마십시오. 공고에 적힌 bullet point(-, ·, 1. 등) 목록을 "원문 텍스트 그대로" 한 줄씩 배열에 담아야 합니다.
3. 100% IT 공고라면 IT 내용을 모두 빠짐없이 찾아 분리하세요.
4. 비-IT 내용(제조공정, 세무재무, 인사, 서비스기획 등 단순 사무/영업/생산직)은 철저하게 무시하고 결코 결과에 포함하지 마세요.
...
[
  {
    ...
    "자격요건": ["원문 내용 1", "원문 내용 2"],
    "우대사항": ["원문 내용 1", "원문 내용 2"],
    "기술스택": ["Java", "Oracle"]
  }
]
"""
...
def main():
    ...
    for i, batch in enumerate(tqdm(batches, desc="진행률")):
        batch_result = process_batch(batch)
        if batch_result:
            for item in batch_result:
                if not isinstance(item, dict):  # Error handling added here check!
                    continue
        ...
        if len(results_list) > 0 and i % 2 == 0:
