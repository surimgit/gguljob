  ...
  const toggleStack = (category: string, stack: string) =>
    setForm((prev) => {
      ...
      return {
        ...
        techStacks: {
          ...prev.techStacks,
          [category]: cur.includes(stack)
            ? cur.filter((s) => s !== stack)
            : [...cur, stack],
        },
      };
    });
    ...
    try {
      ...
      if (form.members.length > 0) {
        await Promise.allSettled(
          form.members.map((m) =>
            inviteUser(projectId, m.userId, { role: m.position }),
          ),
        );
      }
          ...
          >
            ...
            <div className="flex gap-2">
              <input
                ...
                onBlur={(e) =>
                  (e.target.style.borderColor = verifiedUser
                    ? "var(--color-primary)"
                    : "var(--color-border)")
                }
              />
              <button
                ...
                style={{
                  ...
                  cursor:
                    memberDraft.email.trim() && !verifying
                      ? "pointer"
                      : "not-allowed",
                }}
              >
            ...
            <div ref={positionRef} className="relative mt-3">
              <button
                ...
                style={{
                  ...
                  borderColor: positionOpen
                    ? "var(--color-primary)"
                    : "var(--color-border)",
                  color: memberDraft.position
                    ? "var(--color-text-primary)"
                    : "var(--color-text-tertiary)",
                }}
              >
                <span>
                  {memberDraft.position
                    ? getRoleDisplayName(memberDraft.position)
                    : "초대할 포지션 선택"}
                </span>
                <ChevronDown
              ...
              {positionOpen && (
                <ul
                  ref={(node) => {
                    (
                      positionListRef as React.MutableRefObject<HTMLUListElement | null>
                    ).current = node;
                    if (node) node.focus();
                  }}
                  id="position-listbox"
                  ...
                  tabIndex={-1}
                  onKeyDown={(e) => {
                    const idx = memberDraft.position ? ROLE_LIST.indexOf(memberDraft.position) : -1;
                    if (e.key === "ArrowDown") {
                      e.preventDefault();
                      setMemberDraft((prev) => ({
                        ...prev,
                        position:
                          ROLE_LIST[Math.min(idx + 1, ROLE_LIST.length - 1)],
                      }));
                    } else if (e.key === "ArrowUp") {
                      e.preventDefault();
                      setMemberDraft((prev) => ({
                        ...prev,
                        position: ROLE_LIST[Math.max(idx - 1, 0)],
                      }));
                    } else if (e.key === "Enter") {
                      e.preventDefault();
                      setPositionOpen(false);
