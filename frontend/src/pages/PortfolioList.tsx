    ...
    >
              ...
              {!isLoading && portfolios.length > 0 && portfolios.slice(0, 2).map((latest) => (
                ...
                >
                  <div className="-ml-1 self-start rounded-[8px] px-[8px] py-[2px] bg-[rgba(99,102,241,0.12)]">
                    <p className="text-base font-semibold leading-[15px]" style={{ color: '#6366f1' }}>
                      최근 포트폴리오
                    </p>
                  </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 mt-6">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-text-tertiary">
            ...
            <p className="text-base">트러블슈팅을 선택하여 AI 포트폴리오를 만들어보세요</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {portfolios.map((item) => (
                ...
                <div className="flex flex-col gap-3 p-5 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-auto pt-2">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-sm font-bold border border-border bg-white hover:bg-background transition-colors text-text-secondary disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center justify-center px-3 py-2 rounded-xl text-sm font-bold border border-red-200 bg-white hover:bg-red-50 transition-colors text-red-500"
                    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-7 py-4 border-b border-border flex-shrink-0">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-bold border border-border hover:bg-background transition-colors text-text-secondary disabled:opacity-50"
            >
              <Download className="w-3.5 h-3.5" />
              다운로드
            </button>
        ...
        <div className="overflow-y-auto flex-1 px-8 py-6">
          {mdLoading ? (
            <div className="flex items-center justify-center py-20 text-text-tertiary text-base">
              불러오는 중...
            </div>
          ) : (
            <div className="text-base leading-relaxed">
              <Markdown
                ...
                components={{
                  ...
                  p: ({ children }) => <p className="text-base leading-relaxed mb-3" style={{ color: 'var(--color-text-secondary)' }}>{children}</p>,
                  ul: ({ children }) => <ul className="list-disc pl-5 mb-3 text-base" style={{ color: 'var(--color-text-secondary)' }}>{children}</ul>,
                  ol: ({ children }) => <ol className="list-decimal pl-5 mb-3 text-base" style={{ color: 'var(--color-text-secondary)' }}>{children}</ol>,
        ...
        <div className="px-8 py-6">
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={isDeleting}
              className="flex-1 py-2.5 rounded-xl border-2 border-border text-base font-semibold text-text-secondary hover:bg-background transition-colors disabled:opacity-50"
            >
              취소
            </button>
            <button
              ...
              disabled={isDeleting}
              className="flex-1 py-2.5 rounded-xl text-base font-semibold text-white bg-red-500 hover:bg-red-600 transition-colors disabled:opacity-50"
            >
              {isDeleting ? '삭제 중...' : '삭제'}
            </button>
          </div>
