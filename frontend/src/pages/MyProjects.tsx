...
const AVATAR_COLORS = ['var(--color-primary)', 'var(--color-blue)', 'var(--color-success)', '#EC4899'];
...
const ProjectCard = ({ project }: { project: Project }) => {
  const domainColor = DOMAIN_COLORS[project.domain] ?? 'var(--color-primary-hover)';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs font-bold" style={{ color: domainColor }}>{project.domain}</span>
        <span className="flex items-center gap-1 text-xs font-medium" style={{ color: isActive ? 'var(--color-success)' : 'var(--color-text-tertiary)' }}>
          <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: isActive ? 'var(--color-success)' : 'var(--color-text-tertiary)' }} />
          {isActive ? '진행중' : '완료'}
        </span>
      </div>
      ...
      {/* 본문 */}
      <div>
        <h3 className="text-lg font-bold" style={{ color: 'var(--color-text-primary)' }}>{project.title}</h3>
        <p className="text-sm leading-relaxed mt-1" style={{ color: 'var(--color-text-secondary)' }}>{project.description}</p>
      </div>
      ...
      <div className="flex items-center justify-between mt-auto pt-2">
        <div className="flex items-center gap-2 text-xs font-bold">
          <span style={{ color: 'var(--color-blue)' }}>FE {project.fe.current}{project.fe.total != null && `/${project.fe.total}`}</span>
          <span style={{ color: 'var(--color-success)' }}>BE {project.be.current}{project.be.total != null && `/${project.be.total}`}</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>{project.leader.name}</span>
        </div>
      </div>

      {/* MR / 완료일 */}
      <p className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>
        {isActive ? `최근 MR: ${project.lastMrAt}` : `완료: ${project.completedAt}`}
      </p>
    </div>
  );
};
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6 sm:py-10">
        ...
        <div className="flex gap-2 mt-6 mb-4">
          <button
            ...
            style={{
              backgroundColor: tab === 'active' ? 'var(--color-primary)' : 'var(--color-surface)',
              color: tab === 'active' ? 'var(--color-surface)' : 'var(--color-text-secondary)',
          ...
          >
            ...
            <span
              ...
              style={{
                backgroundColor: tab === 'active' ? 'rgba(255,255,255,0.3)' : 'var(--color-border)',
                color: tab === 'active' ? 'white' : 'var(--color-text-tertiary)',
              }}
            >
          ...
          <button
            ...
            style={{
              backgroundColor: tab === 'done' ? 'var(--color-primary)' : 'var(--color-surface)',
              color: tab === 'done' ? 'var(--color-surface)' : 'var(--color-text-secondary)',
          ...
          >
            ...
            <span
              className="text-xs px-1.5 rounded-full"
              style={{
                backgroundColor: tab === 'done' ? 'rgba(255,255,255,0.3)' : 'var(--color-border)',
                color: tab === 'done' ? 'white' : 'var(--color-text-tertiary)',
              }}
            >
              {doneProjects.length}
            </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          ...
          {tab === 'active' && (
            ...
            >
              ...
              >
                <Plus className="w-6 h-6" style={{ color: 'var(--color-text-tertiary)' }} />
              </span>
