...
function FilterButton({ text, selected, onClick }: { text: string; selected: boolean; onClick: () => void }) {
    return (
        <button
            ...
            style={selected ? { backgroundImage: 'linear-gradient(150.6deg, #F7C948 0%, #F2B705 100%)' } : {}}
        >
            {text}
        </button>
    );
        ...
        >
            ...
            <div className="flex items-center gap-3 sm:gap-5 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[15px] sm:text-[17px] font-extrabold text-text-primary mb-1 truncate">
                        {job.title}
                    </p>
...
const JobListingSection = ({ allJobs, allJobsLoaded, bookmarkedIds, onToggleBookmark }: JobListingSectionProps) => {
    ...
    const finalFilteredJobs = useMemo(() => {
        ...
        return jobsFilteredByStack.filter(
            (job) =>
