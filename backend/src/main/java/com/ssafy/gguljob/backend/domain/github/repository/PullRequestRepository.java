...
public interface PullRequestRepository extends JpaRepository<PullRequest, Long> {
    ...
    @Query("SELECT new com.ssafy.gguljob.backend.domain.project.dto.PrItem(" +
        "p.id, p.prNumber, p.title, p.status, p.diffUrl, p.githubCreatedAt, p.githubClosedAt) " +
        "FROM PullRequest p " +
    ...
    @Query("SELECT new com.ssafy.gguljob.backend.domain.project.dto.PrItem(" +
        ...
        "WHERE p.project.id = :projectId AND p.user.id = :userId " +
        "ORDER BY COALESCE(p.githubClosedAt, p.githubCreatedAt) DESC")
    Page<PrItem> findPagedMyPrItems(
    ...
    @Query("SELECT DISTINCT pr.user.id, pr.user.userName, pr.user.profileImageUrl, COUNT(pr.id) " +
        "FROM PullRequest pr " +
    ...
    List<Object[]> findNonMemberContributors(@Param("projectId") Long projectId);
}
